"""Identifier spellings, qualifiers and the orderings built from them.

A request names symbols in several shapes: bare (``get``), qualified
(``Session.get``, ``Router::route``), with parameter types
(``fromJson(JsonReader, TypeToken)``). The helpers here split those shapes
into what the SQL lanes look up, pin hits to the scope a qualifier names, and
order same-named declarations by the other names the request spells.
"""
...
# Words that look like identifiers but never name a project symbol worth a
# definition lookup: keywords, builtins, common locals and English filler.
IDENTIFIER_NOISE = frozenset(
    """
    ...
    for identifier in identifiers:
        ...
        for separator in QUALIFIER_SEPARATORS:
            if separator in identifier:
                scope, leaf_name = identifier.rsplit(separator, 1)
                scope_leaf = scope.rsplit(separator, 1)[-1]
                if leaf_name and scope_leaf:
                    if leaf_name not in lookup:
                        lookup.append(leaf_name)
                    bucket = qualifiers.setdefault(leaf_name, [])
...
def resolve_qualified_hits(
    ...
    *,
    ...
    def structural(hit: SearchHit) -> bool:
        if any(word_in(q, hit.context or "") for q in wanted):
            return True
        components = path_components(hit.path)
    ...
    def declared(hit: SearchHit) -> bool:
        for line in hit.content.splitlines():
            if (
                ...
                and any(word_in(leaf_name, line) for leaf_name in leaves)
                and any(word_in(q, line) for q in wanted)
            ):
                return True
        return False

    def textual(hit: SearchHit) -> bool:
        return any(word_in(q, hit.content) for q in wanted)
...
def pin_definitions_to_qualifiers(
    ...
    *,
    ...
    """Apply every qualifier to the declarations of its own leaf.

    Declarations of a leaf the request qualified are filtered by that scope;
    every other declaration passes through untouched, so a bare name asked
    for alongside a qualified one keeps all of its candidates.
    """
    for leaf_name, scopes in qualifiers.items():
        ...
        kept = {
            ...
            for item in resolve_qualified_definitions(
                pinned, {leaf_name: scopes}, strict=strict
            )
        }
        definitions = [
            item
...
def order_by_comentions(hits: list[SearchHit], names: Sequence[str]) -> list[SearchHit]:
    ...
    def mentions(hit: SearchHit) -> int:
        ...
        return sum(word_in(name, text) for name in names)

    return sorted(hits, key=lambda hit: -mentions(hit))


def signature_text(lines: Sequence[str], max_lines: int = 4) -> str:
...
def order_by_signature_comentions(
    ...
    for definition in definitions:
        ...
        if offset < 0 or offset >= len(lines):
            continue
        signature = signature_text(lines[offset:])
        count = sum(word_in(name, signature) for name in names)
